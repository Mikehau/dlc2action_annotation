...
# at top of your cluster_annotation.py (or utils.py), before any pims imports:
warnings.filterwarnings(
    "ignore",
    message="VideoStream.seek is deprecated",
...
class VideoWindow(QWidget):
    ...
    def __init__(self, window, stack, shape, points_df, frames,
                 n, color, label_str, label_dict, start, end):
        super().__init__()
        self.window       = window       # the MainWindow
        self.n            = n            # index of this clip
        self.label_dict   = label_dict   # shared dict: int→str
        self.border_color = 255 * color

        # precompute strings for display
        self.start_string = self.frame_to_min_sec(start)
        self.end_string   = self.frame_to_min_sec(end)

        # — Main layout & title —
        self.layout = QVBoxLayout(self)
        self.setLayout(self.layout)

        # title of the clip’s current behavior
        self.label = QLabel(label_str)
        self.layout.addWidget(self.label)

        # time bar just under the title
        time_layout = QHBoxLayout()
        time_layout.addWidget(QLabel(f"Start: {self.start_string}"))
        time_layout.addStretch()
        time_layout.addWidget(QLabel(f"End:   {self.end_string}"))
        ...
        self.behavior_scroll.setWidget(self.behavior_group)
        self.behavior_scroll.setFixedHeight(150)
        self.layout.addWidget(self.behavior_scroll)

        # track checkboxes by name
        self.behavior_checkboxes = {}

        # build a set of atomic behaviors from every composite value
        atomic = set()
        for composite in self.label_dict.values():
            parts = [p.strip() for p in composite.split("+")]
            atomic.update(parts)

        # now add one checkbox per atomic behavior
        for name in sorted(atomic):
            self._make_checkbox(name)

        # pre-check those in the incoming label_str
        for piece in label_str.split(" + "):
            cb = self.behavior_checkboxes.get(piece)
            if cb:
                cb.setChecked(True)

        # wire them all so that any change calls _on_any_toggle()
        for cb in self.behavior_checkboxes.values():
            cb.stateChanged.connect(self._on_any_toggle)

        # — Add‐new‐behavior controls —
        add_layout = QHBoxLayout()
        self.new_behavior_edit   = QLineEdit()
        self.new_behavior_edit.setPlaceholderText("New behavior name")
        self.new_behavior_button = QPushButton("Add Behavior")
        self.new_behavior_button.clicked.connect(self._on_add_clicked)
        add_layout.addWidget(self.new_behavior_edit)
        add_layout.addWidget(self.new_behavior_button)
        self.layout.addLayout(add_layout)

    def frame_to_min_sec(self, frame):
        frame_sec = int(frame / 20)
        hours     = frame_sec // 3600
        frame_sec = frame_sec % 3600
        minutes   = frame_sec // 60
        seconds   = frame_sec % 60
    ...
    def _make_checkbox(self, name):
        if name in self.behavior_checkboxes:
            return
        cb = QCheckBox(name)
        self.behavior_layout.addWidget(cb)
        self.behavior_checkboxes[name] = cb
        return cb

    def _on_add_clicked(self):
        name = self.new_behavior_edit.text().strip()
        if not name:
            return
        cb = self._make_checkbox(name)
        cb.setChecked(True)
        cb.stateChanged.connect(self._on_any_toggle)
        self.new_behavior_edit.clear()
        self._on_any_toggle()

    def _on_any_toggle(self):
        # existing toggle logic unchanged…
        unk = self.behavior_checkboxes.get("unknown")
        if unk and unk.isChecked():
            others = [cb for nm, cb in self.behavior_checkboxes.items()
                      if nm != "unknown" and cb.isChecked()]
            if others:
                unk.blockSignals(True)
                unk.setChecked(False)
                unk.blockSignals(False)

        parts = [nm for nm, cb in self.behavior_checkboxes.items() if cb.isChecked()]
        if not parts:
            parts = ["unknown"]
            unk = self.behavior_checkboxes.get("unknown")
            if unk:
        ...
        if new_label in inv:
            ...
            existing = [k for k in self.label_dict.keys() if isinstance(k, int)]
            key = max(existing) + 1 if existing else 0
            self.label_dict[key] = new_label

        self.window.labels[self.n] = key
        self.label.setText(new_label)
        self.window.plot_clusters()
...
class Console(QWidget):
    def __init__(self, window):
        ...
        self.save_full_button.clicked.connect(self.window.save_full)
        ...
        self.remain_button.setDisabled(True)
        ...
        self.radio_layout.addWidget(self.save_full_button)
...
class MainWindow(QWidget):
    ...
    def clicked(self, point, ev):
        ...
        label_idx = self.labels[n]
        label_str = self.label_dict.get(label_idx, "unknown")
    ...
    def get_data(self):
        if sum(x is not None for x in self.feature_files) == 0:
            ...
            self.labels_path = os.path.join(self.annotation_folder, "cluster_labels.pkl")

            if os.path.exists(self.labels_path):
                # load saved labels & dict
                ...
                for video in videos:
                    ...
                    for v, s, e, clip in self.frames:
                        main_labels = annotation.main_labels(s+k-1, e-k+1, clip)
                        self.labels.append(main_labels)
                labels = list(set(self.labels))
                self.label_dict = {i: x for i, x in enumerate(labels)}
                inv_dict = {x: i for i, x in enumerate(labels)}
    ...
    def make_brush(self, x, n):
        full_label = self.label_dict.get(x, "unknown")

        # unknown → light gray; otherwise pick from our label_color_map
        if full_label == "unknown":
            col = QColor("gray")
            col.setAlphaF(0.2)
        else:
            col = self._label_color_map.get(full_label, QColor("gray"))

        # highlight chosen
        if n in self.chosen and full_label not in ("unknown", -100):
            col.setAlphaF(0.7)
    ...
    def plot_clusters(self):
        ...
        # Build a distinct color map for each composite label (excluding "unknown")
        labels = [lab for lab in self.label_dict.values() if lab != "unknown"]
        unique = list(dict.fromkeys(labels))  # preserve order
        self._label_color_map = {
            lab: pg.intColor(i, hues=len(unique))
            for i, lab in enumerate(unique)
        }

        # Scatter brushes & pens
        ...
        # Overlay “x” markers for open videos
        overlay_brush = [
            QBrush(QColor(*vw.color())) 
            for vw in self.open_videos.values()
        ]
        ...
        if hasattr(self, 'legend'):
            try:
                self.graph.getPlotItem().scene().removeItem(self.legend)
            except Exception:
                pass

        # create and attach new legend
        self.legend = pg.LegendItem(size=(100, 20), offset=(10, 10))
        self.legend.setParentItem(self.graph.getPlotItem())

        # add one entry per behavior color
        for lab, col in self._label_color_map.items():
            sym = pg.ScatterPlotItem(
                pos=[(0, 0)],           # dummy
                brush=col,
    ...
    def autolabel(self):
        self.close()
        del self.data
        behaviors = set(self.get_behaviors())
        behaviors.update(self.label_dict.values())
...
@click.option("--sampling", default=40)
@click.option("--clip_length", default=200)
